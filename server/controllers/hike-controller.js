    ...
    */
    ...
    if (typeof(req.session.userId) == 'undefined') {
      ...
      User.findOne({
        ...
        .populate({
          path: 'hikes',
          // Get post and pre-trip
          populate: {
            path: 'preTrip postTrip',
          },
          options: {
            sort: '-updatedAt',
            limit: 3,
      ...
      Hike.findOne({
        ...
        .then(function(foundHike) {
          ...
          if (foundHike.postTrip) {
            var deletePostTrip = PostTrip.findOneAndRemove({
              _id: foundHike.postTrip._id
            })
            deletePostTrip.exec();
          }
          ...
          if (foundHike.preTrip) {
            var deletePreTrip = PreTrip.findOneAndRemove({
              _id: foundHike.preTrip._id
            })
            deletePreTrip.exec();
          }

          var deleteHike = Hike.findOneAndRemove({
            _id: foundHike._id
          })
          deleteHike.exec();
          return res.json({
            message: "Success deleting hike."
          });
        })
        .catch(function(err) {
